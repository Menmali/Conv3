...
def generate_charts(
    ...
    def wrap_label(attributes, max_per_line=3):
        ...
        for i in range(0, len(attributes), max_per_line):
            ...
            line = " × ".join(chunk)
            if i > 0:
                line = "× " + line   # add leading '× ' for continuation lines
            lines.append(line)
        return "\n".join(lines)

    # Slightly wider figure to accommodate multi-line labels
    ...
    # ── Top row: full-width four-curve semantic chart ─────────────────────────
    ...
    # Raw coverage (% of full Cartesian space observed)
    y_raw  = [s.score * 100 for s in stages]

    # Theoretical maximum = min(n_records, expected_combos) / expected_combos
    # (the best any dataset of this size can achieve by uniform sampling)
    ...
    # Normalized score = raw / ceiling  (what % of achievable is covered)
    y_norm = []
    for raw, ceil in zip(y_raw, y_ceil):
        y_norm.append((raw / ceil * 100) if ceil > 0 else 0.0)

    # Random baseline = expected unique draws when sampling n from C combos
    # Uses coupon-collector approximation: E[unique] = C*(1 - ((C-1)/C)^n)
    y_rand = []
    for s in stages:
        C = s.expected_combinations
        if C > 0 and n > 0:
            expected_unique = C * (1 - ((C - 1) / C) ** n) if C > 1 else min(n, 1)
            y_rand.append(expected_unique / C * 100)
        else:
            y_rand.append(0.0)

    # ── Draw the four curves ──────────────────────────────────────────────────
    # 1. Ceiling (theoretical maximum for this dataset size)
    ...
    # ── Annotate Stage 6 with dual callout ────────────────────────────────────
    last_stage = stages[-1]
    lx = last_stage.stage
    raw_final  = y_raw[-1]
    norm_final = y_norm[-1]
    ...
    # ── Annotate all stages with Raw Score only ───────────────────────────────
    ...
    # ── Annotate Stage 6 with Raw Score + Theoretical Ceiling ─────────────────
    last_stage = stages[-1]
    lx = last_stage.stage
    raw_final = y_raw[-1]
    ceil_final = y_ceil[-1]
    ...
    # Remove left margin – first tick will be at the plot edge
    ax1.set_xlim(x[0], x[-1] + 0.3)   # tiny left padding (0.1) to avoid marker clipping
    ax1.margins(x=0)
    ...
    s_col  = [GREEN if r.adequate
              else (ORANGE if r.covered else RED)
              for r in subs]
    yp = range(len(subs))

    ax2.barh(list(yp), s_cnt, color=s_col,
             edgecolor="#ffffff15", height=0.6)
    min_req = subs[0].min_required if subs else 5
    ax2.axvline(min_req, color=ORANGE, linestyle="--",
                linewidth=1.2, alpha=0.8, label=f"Min required ({min_req})")
    ...
    ax2.set_title("Sub-class Population",
    ...
    # THIS LINE FIXED THE ERROR (using 'v' instead of 'val')
    ...
    # ... rest of your existing ax3 code
    ...
    for sp in ax3.spines.values():
        sp.set_edgecolor("#ffffff20")
    ax3.legend(facecolor=BG, edgecolor="#ffffff30",
               labelcolor="white", fontsize=8)

    fig.suptitle(
        "Step 5 — Ontology Mapping  |  Disease Coverage Assessment",
