...
def export_owl(
    ...
    for attr in ontology.attributes:
        parent   = EX[_PARENT.get(attr.id, "ClinicalFinding")]
    ...
    out_path = os.path.join(output_dir, "heart_disease_ontology.owl")
...
def validate_owl(
    ...
    All SPARQL results go through _sparql_rows() → List[List[str]].
    Downstream code only deals with plain str, int, float — no rdflib types.
    """
    g = Graph()
    ...
    for col, bins in bins_by_col.items():
        ...
        if len(out_of_range) > 0:
            violations.append(OWLViolation(
                ...
                column=col,
                description=(
                    f"OWL defines valid range [{global_lo}, {global_hi}] for '{col}'. "
                    f"{len(out_of_range)} value(s) fall outside all defined bins."
                ),
    ...
    for col, raw_valid in codes_by_col.items():
        if col not in df.columns:
            continue
        valid_norm: set = {_norm_code(c) for c in raw_valid}
        numeric         = pd.to_numeric(df[col], errors="coerce").dropna()
        obs_norm: set   = {_norm_code(str(v)) for v in numeric}
        unexpected: set = obs_norm - valid_norm
        if unexpected:
    ...
    # SPARQL FILTER compares the two annotations directly inside the query.
    # Only subclasses that fail appear in results — no Python comparison needed.
    ...
    for r in r4_rows:
        ...
        violations.append(OWLViolation(
            ...
            description=(
                ...
                f"(EPV ≥ 20, Riley et al. 2019). Dataset has {count}."
            ),
            affected_rows=count,
        ))
    if not r4_rows:
